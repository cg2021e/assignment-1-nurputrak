// Name : Nur Putra Khanafi
// NRP : 05111940000020
// Class : Computer Graphics E
...
class WebGLWorld {
  ...
  function main() {
    ...
    EraserLeftObject.transform.position = [-0.6, -0.3, 3];
    EraserLeftObject.transform.rotation = [-90, 2, 95];
    EraserLeftObject.transform.scale = [0.2, 0.2, 0.2];
    EraserLeftObject.lightning.shininessConstant = 5; // Plastic Shininess
  
    // Create Eraser at right pos
    let EraserRightObject = new WebGLObject(gl, EraserModel, vertexShaderSource, fragmentShaderSource);
    EraserRightObject.transform.position = [0.4, -0.25, 3];
    EraserRightObject.transform.rotation = [-90, -3, -5];
    EraserRightObject.transform.scale = [0.2, 0.2, 0.2];
    EraserRightObject.lightning.shininessConstant = 200; // Metal Shininess
  
    let world = new WebGLWorld(gl);
    
    world.clearColor = [0.7, 0.8, 0.9, 1];
    world.camera.position = [0, 1, 5];
    world.camera.up = [0, 1, 0];
    world.lightning.ambientIntensityGlobal = 0.220; // 0.200 + NRP 3 Last Digit (020)
